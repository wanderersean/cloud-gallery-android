...
class GetMediaAsynctask(
    ...
    val showAll: Boolean, val filterUploadedPhotos: Boolean = false, val callback: (media: ArrayList<ThumbnailItem>, isFinal: Boolean) -> Unit
) :
    AsyncTask<Void, ArrayList<ThumbnailItem>, ArrayList<ThumbnailItem>>() {
    ...
    override fun doInBackground(vararg params: Void): ArrayList<ThumbnailItem> {
        ...
        if (filterUploadedPhotos) {
            media = CloudUploadFilter.filterMedia(context, media)
        }
    ...
    override fun onProgressUpdate(vararg values: ArrayList<ThumbnailItem>) {
        super.onProgressUpdate(*values)
        if (values.isNotEmpty()) {
            callback(values[0], false)
        }
    }
