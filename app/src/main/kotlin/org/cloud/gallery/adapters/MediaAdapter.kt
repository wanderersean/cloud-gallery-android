...
class MediaAdapter(
    ...
    private var titleDisplayMode = config.titleDisplayMode
    ...
    private val pendingQueryPaths = mutableSetOf<String>()
    private val queryHandler = android.os.Handler(android.os.Looper.getMainLooper())
    private val batchQueryRunnable = Runnable {
        if (pendingQueryPaths.isNotEmpty()) {
            val pathsToQuery = pendingQueryPaths.toList()
            pendingQueryPaths.clear()
    ...
    private val taskListener = object : CloudUploadManager.TaskListListener {
        override fun onTaskListUpdated() {
            val tasks = CloudUploadManager.getInstance(activity).uploadTasks
            activity.runOnUiThread {
                // 收集需要刷新的 position，使用精确刷新
                val positionsToRefresh = mutableSetOf<Int>()

                for (i in 0 until recyclerView.childCount) {
                    val child = recyclerView.getChildAt(i)
                    val cloudUploadProgress = child.findViewById<com.google.android.material.progressindicator.CircularProgressIndicator>(R.id.cloud_upload_progress)
                    val mediumName = child.findViewById<android.widget.TextView>(R.id.medium_name)
                    if (cloudUploadProgress != null && mediumName != null) {
                        val path = mediumName.tag as? String ?: continue
                        val activeTask = tasks.find { it.path == path && it.status in listOf(CloudUploadManager.UploadStatus.PENDING, CloudUploadManager.UploadStatus.UPLOADING) }

                        if (activeTask != null) {
                            if (cloudUploadProgress.visibility != android.view.View.VISIBLE) {
                                // 需要完整刷新这个 item
                                ...
                                // 只更新进度，不需要刷新整个 item
                                cloudUploadProgress.progress = activeTask.progress
                            }
                        } else if (cloudUploadProgress.visibility == android.view.View.VISIBLE) {
                            // 上传完成，需要刷新这个 item
                            val position = getPositionByPath(path)
                            if (position >= 0) {
                                positionsToRefresh.add(position)
    ...
    private val statusListener = object : CloudStatusManager.StatusCallback {
        override fun onStatusUpdated(path: String, isUploaded: Boolean?) {
            activity.runOnUiThread {
                // 只刷新状态变化的 item
                notifyItemChangedByPath(path)
    ...
    init {
        ...
        CloudUploadManager.getInstance(activity).addTaskListListener(taskListener)
        CloudStatusManager.getInstance(activity).addStatusListener(statusListener)
    }
    ...
    override fun prepareActionMode(menu: Menu) {
        ...
        menu.apply {
            ...
            findItem(R.id.cab_cloud_actions).isVisible = !isInRecycleBin
    ...
    override fun actionItemPressed(id: Int) {
        ...
        when (id) {
            ...
            R.id.cab_cloud_actions -> showCabCloudActionsMenu()
        }
    }
    ...
    override fun getItemSelectionKey(position: Int) = (media.getOrNull(position) as? Medium)?.path?.lowercase()?.hashCode()

    override fun getItemKeyPosition(key: Int) = media.indexOfFirst { (it as? Medium)?.path?.lowercase()?.hashCode() == key }
    ...
    private fun getItemWithKey(key: Int): Medium? = media.firstOrNull { (it as? Medium)?.path?.lowercase()?.hashCode() == key } as? Medium

    fun updateMedia(newMedia: ArrayList<ThumbnailItem>, forceRefresh: Boolean = false) {
        val thumbnailItems = newMedia.clone() as ArrayList<ThumbnailItem>
        if (forceRefresh || thumbnailItems.hashCode() != currentMediaHash) {
    ...
    fun updateTitleDisplayMode(mode: Int) {
        this.titleDisplayMode = mode
        notifyDataSetChanged()
    }
    ...
    private fun batchEditCloudTitle() {
        val cloudStatusManager = CloudStatusManager.getInstance(activity)
        val selectedItems = getSelectedItems()
        val uploadedItems = selectedItems.filter { 
            cloudStatusManager.isUploaded(it.path) == true 
        }
        ...
        val editText = android.widget.EditText(activity).apply {
            hint = activity.getString(R.string.batch_cloud_title_hint)
            setSingleLine()
        }
                ...
                if (newTitle.isNotEmpty()) {
                    ...
                    CoroutineScope(Dispatchers.IO).launch {
                        val apiService = org.fossify.gallery.cloud.CloudApiService(
                            org.fossify.gallery.cloud.CloudAccountManager.getInstance(activity)
                        )
                        
                        var successCount = 0
                        uploadedItems.forEach { item ->
                            val result = apiService.updateCloudTitle(item.path, newTitle)
                            if (result.isSuccess) {
                                cloudStatusManager.updateTitle(item.path, newTitle)
                                successCount++
                            }
                        }
                        ...
                        withContext(Dispatchers.Main) {
                            notifyDataSetChanged()
                            activity.toast("${activity.getString(R.string.batch_cloud_titles_updated)} ($successCount/${uploadedItems.size})")
                            finishActMode()
                        }
                    }
                }
    ...
    private fun uploadToCloud() {
        ...
        paths.forEach { path ->
            uploadManager.uploadFile(path, object : CloudUploadManager.UploadCallback {
                override fun onUploadStart(path: String) {
                    activity.runOnUiThread {
                        activity.toast(R.string.cloud_uploading)
                    }
                }

                override fun onUploadProgress(path: String, progress: Int) {}

                override fun onUploadSuccess(path: String) {
                    cloudStatusManager.markAsUploaded(path)
                    activity.runOnUiThread {
                        notifyDataSetChanged()
                        activity.toast(R.string.cloud_upload_success)
                    }
                }

                override fun onUploadError(path: String, error: String) {
                    activity.runOnUiThread {
                        activity.toast("${activity.getString(R.string.cloud_upload_failed)}: $error")
                    }
                }
            })
        }
    ...
    private fun uploadSingleFile(path: String, cloudIcon: android.widget.ImageView, medium: Medium) {
        val uploadManager = CloudUploadManager.getInstance(activity)
        val cloudStatusManager = CloudStatusManager.getInstance(activity)
    ...
    private fun showCloudActionsMenu(anchor: android.view.View, medium: Medium) {
        org.fossify.gallery.helpers.CloudActionsHelper.showCloudActionsMenu(activity, medium) {
            notifyDataSetChanged()
        }
    }

    private fun showCabCloudActionsMenu() {
        val anchor = activity.window.decorView.findViewById<android.view.View>(R.id.cab_cloud_actions)
            ?: activity.window.decorView
        // 使用 getPopupMenuTheme() 获取与当前主题匹配的 popup 主题
        val popupTheme = activity.getPopupMenuTheme()
        val popupContext = androidx.appcompat.view.ContextThemeWrapper(anchor.context, popupTheme)
        val popup = androidx.appcompat.widget.PopupMenu(popupContext, anchor)
        val idUpload = 1
        val idBatchTitle = 2
        popup.menu.add(0, idUpload, 0, activity.getString(R.string.cloud_upload))
        popup.menu.add(0, idBatchTitle, 1, activity.getString(R.string.batch_edit_cloud_title))
        popup.setOnMenuItemClickListener { item ->
            when (item.itemId) {
                idUpload -> { uploadToCloud(); true }
                idBatchTitle -> { batchEditCloudTitle(); true }
                else -> false
            }
        }
        popup.show()
    ...
    private fun setupThumbnail(view: View, medium: Medium) {
        val isSelected = selectedKeys.contains(medium.path.lowercase().hashCode())
        bindItem(view, medium).apply {
            ...
            val cloudTitle = CloudStatusManager.getInstance(activity).getCloudTitle(medium.path)

            // 根据标题显示模式处理标题显示
            when (titleDisplayMode) {
                TITLE_DISPLAY_MODE_HIDE_ALL -> {
                    mediumName.beGone()
                }
                TITLE_DISPLAY_MODE_SHOW_MERGED -> {
                    mediumName.beVisibleIf(isListViewType || true)
                    mediumName.text = if (!cloudTitle.isNullOrEmpty()) cloudTitle else medium.name
                }
                TITLE_DISPLAY_MODE_CLOUD_ONLY -> {
                    if (!cloudTitle.isNullOrEmpty()) {
                        mediumName.beVisible()
                        mediumName.text = cloudTitle
                    } else {
                        mediumName.beGone()
                    }
                }
            ...
            val isLoggedIn = CloudAccountManager.getInstance(activity).isLoggedIn
            val cloudIcon = cloudStatus
            val cloudContainer = cloudStatusContainer
            val uploadProgress = cloudUploadProgress

            if (isLoggedIn) {
                val uploadManager = CloudUploadManager.getInstance(activity)
                val activeTask = uploadManager.uploadTasks.find { 
                    it.path == medium.path && it.status in listOf(CloudUploadManager.UploadStatus.PENDING, CloudUploadManager.UploadStatus.UPLOADING) 
                }
                ...
                if (activeTask != null) {
                    ...
                    uploadProgress?.setOnLongClickListener {
                        ...
                        showCloudActionsMenu(uploadProgress, medium)
                        true
                    }
                } else {
                    val cloudStatusManager = CloudStatusManager.getInstance(activity)
                    val cachedStatus = cloudStatusManager.isUploaded(medium.path)
                    
                    if (cachedStatus != null) {
                        cloudContainer?.beVisible()
                        cloudIcon?.beVisible()
                        uploadProgress?.beGone()
                        
                        if (cachedStatus) {
                            // 云端收藏状态 → 显示绿色爱心；否则显示绿色云盘
                            ...
                            cloudIcon?.setOnLongClickListener {
                                if (selectedKeys.isNotEmpty()) {
                                    return@setOnLongClickListener false
                                }
                                
                                it.performHapticFeedback(android.view.HapticFeedbackConstants.LONG_PRESS)
                                showCloudActionsMenu(cloudIcon, medium)
                                true
                            }
                        } else {
                            ...
                            cloudIcon?.setOnLongClickListener(null)
                        }
                    } else {
                        cloudContainer?.beGone()
                        cloudIcon?.beGone()
                        uploadProgress?.beGone()
                        
                        if (!cloudStatusManager.isPending(medium.path)) {
                            pendingQueryPaths.add(medium.path)
                            queryHandler.removeCallbacks(batchQueryRunnable)
                            queryHandler.postDelayed(batchQueryRunnable, 100)
                        }
                    }
                }
            } else {
    ...
    fun cleanup() {
        queryHandler.removeCallbacks(batchQueryRunnable)
        pendingQueryPaths.clear()
        CloudStatusManager.getInstance(activity).removeStatusListener(statusListener)
    }

    /**
     * 根据 path 查找对应的 adapter position
     * @return position，如果未找到返回 -1
     */
    private fun getPositionByPath(path: String): Int {
        val lowerPath = path.lowercase()
        return media.indexOfFirst { (it as? Medium)?.path?.lowercase() == lowerPath }
    }

    /**
     * 根据 path 精确刷新对应的 item，避免全量刷新导致的闪烁
     */
    private fun notifyItemChangedByPath(path: String) {
        val position = getPositionByPath(path)
        if (position >= 0) {
            notifyItemChanged(position)
        }
