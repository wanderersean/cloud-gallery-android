...
class DirectoryAdapter(
    ...
    override fun prepareActionMode(menu: Menu) {
        ...
        menu.apply {
            ...
            findItem(R.id.cab_cloud_actions).isVisible = org.fossify.gallery.cloud.CloudAccountManager.getInstance(activity).isLoggedIn
    ...
    private fun showCabCloudActionsMenu() {
        ...
        val idUpload = 1
        val idBatchTitle = 2
        popup.menu.add(0, idUpload, 0, activity.getString(org.fossify.gallery.R.string.cloud_upload))
        popup.menu.add(0, idBatchTitle, 1, activity.getString(org.fossify.gallery.R.string.batch_edit_cloud_title))
        popup.setOnMenuItemClickListener { item ->
            when (item.itemId) {
                idUpload -> { uploadSelectedToCloud(); true }
                idBatchTitle -> { batchEditCloudTitleForDirs(); true }
                else -> false
            }
        }
        popup.show()
    ...
    private fun uploadSelectedToCloud() {
        ...
        val uploadManager = org.fossify.gallery.cloud.CloudUploadManager.getInstance(activity)
        val cloudStatusManager = org.fossify.gallery.cloud.CloudStatusManager.getInstance(activity)
        paths.forEach { dirPath ->
            val dir = java.io.File(dirPath)
            dir.walkTopDown().filter { it.isFile && it.extension.lowercase() in listOf("jpg", "jpeg", "png", "gif", "mp4", "mov", "webp", "heic") }.forEach { file ->
                uploadManager.uploadFile(file.absolutePath, object : org.fossify.gallery.cloud.CloudUploadManager.UploadCallback {
                    override fun onUploadStart(path: String) {}
                    override fun onUploadProgress(path: String, progress: Int) {}
                    override fun onUploadSuccess(path: String) { cloudStatusManager.markAsUploaded(path) }
                    override fun onUploadError(path: String, error: String) {}
                })
            }
        }
        activity.toast(org.fossify.gallery.R.string.cloud_uploading)
        finishActMode()
    }

    private fun batchEditCloudTitleForDirs() {
        activity.toast(org.fossify.gallery.R.string.batch_edit_cloud_title)
        finishActMode()
    }
