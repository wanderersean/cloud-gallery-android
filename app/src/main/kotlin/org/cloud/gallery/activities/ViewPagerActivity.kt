...
class ViewPagerActivity : BaseViewerActivity(), ViewPager.OnPageChangeListener, ViewPagerFragment.FragmentListener {
    ...
    private val cloudUploadTaskListener = object : org.fossify.gallery.cloud.CloudUploadManager.TaskListListener {
        override fun onTaskListUpdated() {
            runOnUiThread {
                val currentMedium = getCurrentMedium() ?: return@runOnUiThread
                updateCloudButtonState(currentMedium.path)
            }
        }
    }
    ...
    override fun onResume() {
        ...
        if (currentMedium != null) {
            val cloudTitle = org.fossify.gallery.cloud.CloudStatusManager.getInstance(this).getCloudTitle(currentMedium.path)
            binding.mediumViewerToolbar.title = cloudTitle ?: currentMedium.path.getFilenameFromPath()
        } else {
            binding.mediumViewerToolbar.title = mPath.getFilenameFromPath()
        }

        org.fossify.gallery.cloud.CloudUploadManager.getInstance(this).addTaskListListener(cloudUploadTaskListener)
    }
    ...
    override fun onDestroy() {
        ...
        org.fossify.gallery.cloud.CloudUploadManager.getInstance(this).removeTaskListListener(cloudUploadTaskListener)
    }
    ...
    private fun toggleFileVisibility(hide: Boolean, callback: (() -> Unit)? = null) {
        toggleFileVisibility(getCurrentPath(), hide) {
            ...
            val cloudStatusManager = CloudStatusManager.getInstance(this)
            val cloudTitle = cloudStatusManager.getCloudTitle(it)
            binding.mediumViewerToolbar.title = cloudTitle ?: newFileName
    ...
    private fun initBottomActionButtons() {
        ...
        val isLoggedIn = CloudAccountManager.getInstance(this).isLoggedIn
        val notInRecycleBin = currentMedium?.getIsInRecycleBin() == false
        binding.bottomActions.bottomCloudUploadWrapper.beVisibleIf(isLoggedIn && notInRecycleBin)
        if (isLoggedIn && notInRecycleBin && currentMedium != null) {
            updateCloudButtonState(currentMedium.path)
        }
        binding.bottomActions.bottomCloudUpload.setOnLongClickListener {
            val medium = getCurrentMedium()
            if (medium != null) {
                val isUploaded = CloudStatusManager.getInstance(this).isUploaded(medium.path) == true
                if (isUploaded) {
                    org.fossify.gallery.helpers.CloudActionsHelper.showCloudActionsMenu(this, medium) {
                        updateCloudButtonState(medium.path)
        ...
        binding.bottomActions.bottomCloudUpload.setOnClickListener {
    ...
    private fun updateBottomActionIcons(medium: Medium?) {
        ...
        val isLoggedIn = org.fossify.gallery.cloud.CloudAccountManager.getInstance(this).isLoggedIn
        val notInRecycleBin = !medium.getIsInRecycleBin()
        binding.bottomActions.bottomCloudUploadWrapper.beVisibleIf(isLoggedIn && notInRecycleBin)
        if (isLoggedIn && notInRecycleBin) {
            updateCloudButtonState(medium.path)
        }
    }
    ...
    private fun updateCloudButtonState(path: String) {
        val cloudStatusManager = CloudStatusManager.getInstance(this)
        val uploadManager = org.fossify.gallery.cloud.CloudUploadManager.getInstance(this)

        val activeTask = uploadManager.uploadTasks.find {
            it.path == path && it.status in listOf(org.fossify.gallery.cloud.CloudUploadManager.UploadStatus.PENDING, org.fossify.gallery.cloud.CloudUploadManager.UploadStatus.UPLOADING)
        }
        ...
        if (activeTask != null) {
            ...
            binding.bottomActions.bottomCloudUploadProgress.setOnLongClickListener {
                ...
                if (currentMedium != null) {
                    ...
                    org.fossify.gallery.helpers.CloudActionsHelper.showCloudActionsMenu(this, currentMedium) {
                        updateCloudButtonState(currentMedium.path)
                        updateActionbarTitle()
                    }
                    true
            ...
            binding.bottomActions.bottomCloudUploadProgress.beGone()
            binding.bottomActions.bottomCloudUploadProgress.setOnLongClickListener(null)
            binding.bottomActions.bottomCloudUpload.beVisible()

            val cachedStatus = cloudStatusManager.isUploaded(path)
            if (cachedStatus != null) {
                if (cachedStatus) {
                    // 云端收藏状态 → 显示绿色爱心；否则显示绿色云盘
                    val isCloudFavorite = cloudStatusManager.isFavorite(path) == true
                ...
                if (!cloudStatusManager.isPending(path)) {
                    cloudStatusManager.queryUploadStatus(listOf(path), object : CloudStatusManager.StatusCallback {
                        override fun onStatusUpdated(queryPath: String, isUploaded: Boolean?) {
                            if (queryPath == path) {
                                runOnUiThread {
                                    binding.bottomActions.bottomCloudUpload.beVisible()
                                    if (isUploaded == true) {
                                        ...
                                        if (cloudTitle != null) {
                                            binding.mediumViewerToolbar.title = cloudTitle
                                        }
    ...
    private fun uploadCurrentToCloud() {
        ...
        val cloudStatusManager = CloudStatusManager.getInstance(this)
        if (cloudStatusManager.isUploaded(path) == true) {
            return
        }

        val uploadManager = CloudUploadManager.getInstance(this)
        uploadManager.uploadFile(path, object : CloudUploadManager.UploadCallback {
            override fun onUploadStart(path: String) {
                runOnUiThread {
                    toast(R.string.cloud_uploading)
                }
            }
            ...
            override fun onUploadSuccess(path: String) {
                ...
                runOnUiThread {
                    binding.bottomActions.bottomCloudUpload.setImageResource(R.drawable.ic_cloud_done_vector)
                    toast(R.string.cloud_upload_success)
                }
            }

            override fun onUploadError(path: String, error: String) {
                runOnUiThread {
                    toast("${getString(R.string.cloud_upload_failed)}: $error")
                }
            }
        })
    }
    ...
    private fun refreshViewPager(refetchPosition: Boolean = false) {
        ...
        if (!isRandomSorting || isExternalIntent()) {
            GetMediaAsynctask(applicationContext, mDirectory, isPickImage = false, isPickVideo = false, showAll = mShowAll, filterUploadedPhotos = CloudUploadFilterState.showOnlyUnuploadedPhotos) { newMedia, isFinal ->
                gotMedia(newMedia, refetchViewPagerPosition = refetchPosition)
            }.execute()
        }
    }
    ...
    private fun updateActionbarTitle() {
        runOnUiThread {
            val medium = getCurrentMedium()
            if (medium != null) {
                val cloudTitle = org.fossify.gallery.cloud.CloudStatusManager.getInstance(this).getCloudTitle(medium.path)
                binding.mediumViewerToolbar.title = cloudTitle ?: medium.path.getFilenameFromPath()
            }
        }
    }
