...
class PickMediumDialog(val activity: BaseSimpleActivity, val path: String, val callback: (path: String) -> Unit) {
    ...
    init {
        ...
        binding.mediaFastscroller.handleWidth = (36 * activity.resources.displayMetrics.density).toInt()
        binding.mediaFastscroller.handleHeight = (56 * activity.resources.displayMetrics.density).toInt()
        ...
        GetMediaAsynctask(activity, path, false, false, false, false) { newMedia, isFinal ->
            gotMedia(newMedia)
        }.execute()
    }
