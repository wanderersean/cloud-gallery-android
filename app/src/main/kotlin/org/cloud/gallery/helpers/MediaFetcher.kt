...
class MediaFetcher(val context: Context) {
    ...
    fun getFilesFrom(
        ...
        if (context.isPathOnOTG(curPath)) {
            ...
            // Also try the filesystem-based scan and merge any files that MediaStore missed.
            // On scoped storage without MANAGE_EXTERNAL_STORAGE, listFiles() returns null and
            // getMediaInFolder will return an empty list, so this is safe.
            val fsMedia = getMediaInFolder(
            ...
            if (curMedia.isEmpty()) {
                curMedia.addAll(fsMedia)
            } else if (fsMedia.isNotEmpty()) {
                // Merge: add filesystem-only files that MediaStore missed
                val existingPaths = curMedia.map { it.path.lowercase() }.toHashSet()
                fsMedia.filter { !existingPaths.contains(it.path.lowercase()) }.forEach { curMedia.add(it) }
            }
        }
