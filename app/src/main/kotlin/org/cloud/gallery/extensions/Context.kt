...
import org.fossify.gallery.helpers.CloudUploadFilter
...
fun Context.movePinnedDirectoriesToFront(dirs: ArrayList<Directory>): ArrayList<Directory> {
    ...
    dirs.forEach {
        if (pinnedFolders.any { pinned -> it.path.equals(pinned, true) }) {
            foundFolders.add(it)
        }
    }
...
fun Context.getSortedDirectories(source: ArrayList<Directory>): ArrayList<Directory> {
    ...
    if (sorting and SORT_BY_RANDOM != 0) {
        ...
        config.customFoldersOrder.split("|||").forEach { path ->
            val index = dirs.indexOfFirst { it.path.equals(path, true) }
            if (index != -1) {
            ...
            ensureBackgroundThread {
                ...
                try {
                    ...
                    cached.forEach { thumbnailItem ->
                        val cachedPath = (thumbnailItem as? Medium)?.path
                        if (cachedPath != null && !newPathsSet.contains(cachedPath.lowercase())) {
                            deleteDBPath(cachedPath)
                        }
                    }
                } catch (ignored: Exception) {
                }
...
fun Context.getCachedMedia(
    ...
    getVideosOnly: Boolean = false,
    getImagesOnly: Boolean = false,
    filterUploadedPhotos: Boolean = false,
    ...
    ensureBackgroundThread {
        ...
        if (filterUploadedPhotos) {
            media = CloudUploadFilter.filterMedia(this, media)
        }
